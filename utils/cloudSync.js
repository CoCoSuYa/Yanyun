...
async function syncUpdateUserToCloud(userId, updates) {
  ...
  try {
    ...
    // 成就相关字段
    if (updates.achievements !== undefined) cloudUpdates.achievements = updates.achievements;

    // 已读标记相关字段
    if (updates.readNoticeIds !== undefined) cloudUpdates.read_notice_ids = updates.readNoticeIds;
    if (updates.readSuggestionIds !== undefined) cloudUpdates.read_suggestion_ids = updates.readSuggestionIds;
